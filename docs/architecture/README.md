...
Clients (iOS and web) send HTTPS requests through Azure Front Door/CDN and AWS load balancers. The API Gateway validates JWTs and applies rate limits before forwarding traffic into a private VPC where **Wayla Api** runs on ECS.
...
|-----------|-------------|
| **Front Door & CDN Profiles** | Azure Front Door (or equivalent CDN). Terminates TLS at the edge, caches static assets, and routes traffic to the origin. |
...
| Area | Technology |
|------|------------|
| CDN / edge | Azure Front Door & CDN Profiles |
| Load balancing | AWS Elastic Load Balancing |
| API management | AWS API Gateway |
| Compute | AWS ECS |
| Identity | Keycloak Auth, Supabase AUTH (external); Auth service (internal) |
